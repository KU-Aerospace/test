...
%-------------------------------------------------------------------------
function [x,iter,error,nstate,xhis,fhis,error_his]=newton_raphason_ndim_FUN2(alpha,epsilon,iter_max,ndim,xo,delx)
%-------------------------------------------------------------------------
...
%-------------------------------------------------------------------------
    x1 = x;
    [fun]=function_ndim_user_FUN2(x1);
%
    for j = 1:ndim
%-------------------------------------------------------------------------
...
%-------------------------------------------------------------------------
        x(j,1) = x1(j,1) + delx;
        [funp]=function_ndim_user_FUN2(x);
%-------------------------------------------------------------------------
% (3) negative perturbation
%-------------------------------------------------------------------------
        x(j,1) = x1(j,1) - delx;
        [funm]=function_ndim_user_FUN2(x);
%
        x(j,1) = x1(j,1);
%-------------------------------------------------------------------------
% (4) function gradient
...
%-------------------------------------------------------------------------
    ...
    for j =1:ndim
        ...
        errorf= errorf+ fun(j,1)^2;
   end
    error = sqrt(error /ndim);
    errorf= sqrt(errorf/ndim);
%
%   error_his(iter,1) = error;
    error_his(iter,1) = errorf;
%
    if(error<epsilon) 
         nstate = 2;
         break;
