...
% 	           cosTheta * cosPhi ];
           ...
           xfm = makehgtform('xrotate',phi,'yrotate',theta,'zrotate',psi);
           transformMatrix=xfm(1:3,1:3);
        ...
        if strcmp(pAnimate,'on') == 1
          if strcmp(pOutput,'none') ~= 1
             ...
             if i == 1;
                imwrite(A,map,pOutput,'gif','LoopCount',Inf,'DelayTime',.1);
             else
                imwrite(A,map,pOutput,'gif','WriteMode','append','DelayTime',.1);
             end;
          end;
        end;
     end;
